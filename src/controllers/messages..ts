...
import { Controller, Ctx, Post, Get, Params, Delete } from 'amala'
...
export default class MessageController {
  ...
  async getMessages() {
    ...
    const msg = await MessageModel.find(filter)
    return msg
  }
  ...
  async createMessage(@Ctx() ctx: Context) {
    ...
    const msg = await MessageModel.create(doc)
  ...
  async getMessageById(@Params('id') id: string) {
    ...
    const msg = await MessageModel.findOne(filter)
    return msg
  }
  ...
  async updateMessageById(@Ctx() ctx: Context, @Params('id') id: string) {
    const filter = { _id: id, name: CURREN_USER_NAME }
    const doc = ctx.request.body.doc
    let msg = await MessageModel.findOneAndUpdate(filter, doc)
  ...
  async deleteMessage(@Params('id') id: string) {
    const filter = { _id: id, name: CURREN_USER_NAME }
    const msg = await MessageModel.findOneAndDelete(filter)
    return msg
  }
}
